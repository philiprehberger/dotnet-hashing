...
/// <summary>
/// Provides enhanced password hashing using PBKDF2-SHA512 with larger salt and hash sizes.
/// Uses the format <c>$pbkdf2v2$iterations$salt$hash</c> to distinguish from v1.
/// </summary>
internal static class EnhancedPasswordHasher
{
    /// <summary>
    ...
    /// <summary>
    /// Hashes a password using PBKDF2-SHA512 with a cryptographically random 256-bit salt.
    /// </summary>
    /// <param name="password">The plaintext password to hash.</param>
    /// <param name="iterations">The number of PBKDF2 iterations.</param>
    /// <returns>A string in the format <c>$pbkdf2v2$iterations$salt$hash</c>.</returns>
    ...
    internal static string Hash(string password, int iterations = DefaultIterations)
    {
        ...
        return $"$pbkdf2v2${iterations}${saltBase64}${hashBase64}";
    }

    /// <summary>
    /// Verifies a password against a previously generated enhanced PBKDF2 hash string.
    /// </summary>
    /// <param name="password">The plaintext password to verify.</param>
    /// <param name="hash">The hash string in <c>$pbkdf2v2$iterations$salt$hash</c> format.</param>
    ...
    internal static bool Verify(string password, string hash)
    {
        ...
        var computed = Rfc2898DeriveBytes.Pbkdf2(password, parts.Salt, parts.Iterations, HashAlgorithmName.SHA512, HashSize);
    ...
    /// <exception cref="FormatException">Thrown when <paramref name="hash"/> is not in the expected format.</exception>
    internal static bool NeedsRehash(string hash, int iterations = DefaultIterations)
    {
        ArgumentNullException.ThrowIfNull(hash);
    ...
    private static (int Iterations, byte[] Salt, byte[] Hash) ParseHash(string hash)
    {
        ...
        if (segments.Length != 4 || segments[0] != "pbkdf2v2")
            throw new FormatException("Hash is not in the expected $pbkdf2v2$iterations$salt$hash format.");
